...
double ImprovedPerlinNoise::noise(double x, double y, double z) {
      ...
      return pLerp(w, pLerp(v, pLerp(u, grad(p[AA  ], x  , y  , z   ),  // AND ADD
                                     grad(p[BA  ], x-1, y  , z   )), // BLENDED
                             pLerp(u, grad(p[AB  ], x  , y-1, z   ),  // RESULTS
                                     grad(p[BB  ], x-1, y-1, z   ))),// FROM  8
                     pLerp(v, pLerp(u, grad(p[AA+1], x  , y  , z-1 ),  // CORNERS
                                     grad(p[BA+1], x-1, y  , z-1 )), // OF CUBE
                             pLerp(u, grad(p[AB+1], x  , y-1, z-1 ),
