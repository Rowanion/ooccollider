...
//							break;
						...
						case WrappedOcNode::ONLINE:
							++onlineCount;
							myVF::getSingleton()->draw((*wIt)->iVbo);
//							(*wIt)->iVbo->draw();
							break;
				...
				for(wIt = mPriReRequestList.begin(); wIt != mPriReRequestList.end(); ++wIt, queryCount++){
					...
					if (mPriOccResults[(*wIt)->octreeNode->getId()] <= 0){
						if ((*wIt)->state == WrappedOcNode::REQUESTED_ONLINE ){
							...
							myVF::getSingleton()->setOffline((*wIt)->iVbo);
//							(*wIt)->iVbo->setOffline();
							(*wIt)->usageCount = 0;
							(*wIt)->offlineCount = 0;
						...
						if ((*wIt)->state == WrappedOcNode::REQUESTED_ONLINE ){
							...
							(*wIt)->state = WrappedOcNode::ONLINE;
//							(*wIt)->iVbo->setOnline();
							myVF::getSingleton()->setOnline((*wIt)->iVbo);
							(*wIt)->usageCount = 0;
							delta = ((*wIt)->iVbo->getVertexCount()*sizeof(V4N4)) + ((*wIt)->iVbo->getIndexCount()*4);
							mPriL2Cache -= delta;
							mPriL1Cache += delta;
...
//	cerr << "sending # of requests: " << mPriRequests.size() << endl;
//	cerr << "sending this round: " << loadCount << endl;
	if (loadCount>0){
...
//			(*wIt)->iVbo->draw(true);
//			myVF::getSingleton()->drawAlt((*wIt)->iVbo);
			myVF::getSingleton()->draw((*wIt)->iVbo, true);
		}
	}
...
void RenderCoreGlFrame::manageCaching()
{
	...
	unsigned onlineCount = 0;
	for (; wIt != mPriWrapperInFrustum.end();){
...
//				(*wIt)->iVbo->setOffline();
			...
			mPriRequests.insert(Quintuple((*wIt)->octreeNode->getLevel(), (*wIt)->dist, MpiControl::getSingleton()->getRank(), (*wIt)->octreeNode->getId(), true));
			mPriRequestedWrapper.push_back(*wIt);
			(*wIt)->state = WrappedOcNode::REQUESTED;
			(*wIt)->iVbo = 0;
			mPriWrapperInFrustum.erase(wIt++);
		}
		else if ((*wIt)->state == WrappedOcNode::REQUESTED){
			mPriRequestedWrapper.push_back(*wIt);
			(*wIt)->iVbo = 0;
			mPriWrapperInFrustum.erase(wIt++);
		}
...
void RenderCoreGlFrame::clearRequests()
{
	...
	WrapperListIter wIt = mPriRequestedWrapper.begin();
	while (wIt != mPriRequestedWrapper.end()){
		if ((*wIt)->state == WrappedOcNode::REQUESTED){
			(*wIt)->state = WrappedOcNode::MISSING;
			(*wIt)->iVbo = 0;
			++wIt;
//			mPriWrapperListSize.erase(wIt++);
		}
		else{
			++wIt;
		}
...
void
RenderCoreGlFrame::reloadOnline()
{
	...
	for (; wli != mPriWrapperInFrustum.end(); ){
		...
		case WrappedOcNode::ONLINE:
//			(*wli)->iVbo->setOnline();
			myVF::getSingleton()->setOnline((*wli)->iVbo);
			wli++;
			break;
		case WrappedOcNode::RETEST_ONLINE:
//			(*wli)->iVbo->setOnline();
		...
		case WrappedOcNode::REQUESTED_ONLINE:
//			(*wli)->iVbo->setOnline();
			myVF::getSingleton()->setOnline((*wli
			)->iVbo);
			wli++;
			break;
		default:
...
RenderCoreGlFrame::clearCache()
{
	...
	for (; wli != mPriWrapperInFrustum.end(); ){
		...
		case WrappedOcNode::REQUESTED_ONLINE:
			...
			break;
		case WrappedOcNode::OFFLINE:
			myVF::getSingleton()->freeVbo((*wli)->iVbo);
			(*wli)->iVbo = 0;
//			delete ((*wli)->iVbo);
//			(*wli)->iVbo = 0;
...
//		cerr << "actually received a bunch of vbos..." << endl;
			...
			if (idLoIt != mPriIdLoMap.end()){
				if (idLoIt->second->getWrapper()->state == WrappedOcNode::REQUESTED){
					...
					if (myVF::getSingleton()->getFreeMem() >= ve.getVboSize(i)){
						...
						idLoIt->second->getWrapper()->state = WrappedOcNode::RETEST_OFFLINE;
						idLoIt->second->getWrapper()->usageCount = 0;
						idLoIt->second->getWrapper()->offlineCount = 0;
...
//				}
		...
		cout << headerS.str() << "Total size of WrapperList: " << mPriWrapperListSize << endl;
		...
		cout << headerS.str() << "FreeMem f/u/t: " << myVF::getSingleton()->getFreeMem() << "/" << myVF::getSingleton()->getUsedMem() << "/" << myVF::getSingleton()->getTotalMem() << endl;
		cout << headerS.str() << "L2_Cache (offi): " << mPriL2Cache/1048576 << "Mib / " << L2_CACHE_THRESHOLD/1048576 << "Mib" << endl;
...
void RenderCoreGlFrame::debug(ooctools::IVbo* _iVbo, VboEvent* _vE, unsigned _idx){
	...
	for (unsigned i=0; i< _vE->getIndexCount(_idx); ++i){
		if (_iVbo->indexData()[i] != _vE->getIndexArray(_idx)[i]){
			...
			cerr << "Index " << i << ": " << _iVbo->indexData()[i] << " vs " << _vE->getIndexArray(_idx)[i] << endl;
		}
	}
	for (unsigned i=0; i< _vE->getVertexCount(_idx); ++i){
		if (_iVbo->vertexData()[i].v[0] != _vE->getVertexArray(_idx)[i].v[0]){
			cerr << "error! v0" << endl;
			cerr << "v0 " << i << ": " << _iVbo->vertexData()[i].v[0] << " vs " << _vE->getVertexArray(_idx)[i].v[0] << endl;
			err = true;
		}
		else if (_iVbo->vertexData()[i].v[1] != _vE->getVertexArray(_idx)[i].v[1]){
			cerr << "error! v1" << endl;
			cerr << "v1 " << i << ": " << _iVbo->vertexData()[i].v[1] << " vs " << _vE->getVertexArray(_idx)[i].v[1] << endl;
			err = true;
		}
		else if (_iVbo->vertexData()[i].v[2] != _vE->getVertexArray(_idx)[i].v[2]){
			cerr << "error! v2" << endl;
			cerr << "v2 " << i << ": " << _iVbo->vertexData()[i].v[2] << " vs " << _vE->getVertexArray(_idx)[i].v[2] << endl;
			err = true;
		}
		else if (_iVbo->vertexData()[i].v[3] != _vE->getVertexArray(_idx)[i].v[3]){
			cerr << "error! v3" << endl;
			cerr << "v3 " << i << ": " << _iVbo->vertexData()[i].v[3] << " vs " << _vE->getVertexArray(_idx)[i].v[3] << endl;
			err = true;
		}
		else if (_iVbo->vertexData()[i].n[0] != _vE->getVertexArray(_idx)[i].n[0]){
			cerr << "error! n0" << endl;
			cerr << "n0 " << i << ": " << _iVbo->vertexData()[i].n[0] << " vs " << _vE->getVertexArray(_idx)[i].n[0] << endl;
			err = true;
		}
		else if (_iVbo->vertexData()[i].n[1] != _vE->getVertexArray(_idx)[i].n[1]){
			cerr << "error! n1" << endl;
			err = true;
			cerr << "n1 " << i << ": " << _iVbo->vertexData()[i].n[1] << " vs " << _vE->getVertexArray(_idx)[i].n[1] << endl;
		}
		else if (_iVbo->vertexData()[i].n[2] != _vE->getVertexArray(_idx)[i].n[2]){
			cerr << "error! n2" << endl;
			err = true;
			cerr << "n2 " << i << ": " << _iVbo->vertexData()[i].n[2] << " vs " << _vE->getVertexArray(_idx)[i].n[2] << endl;
		}
		else if (_iVbo->vertexData()[i].n[3] != _vE->getVertexArray(_idx)[i].n[3]){
			cerr << "error! n3" << endl;
			err = true;
			cerr << "n3 " << i << ": " << _iVbo->vertexData()[i].n[3] << " vs " << _vE->getVertexArray(_idx)[i].n[3] << endl;
		}
	}
	if (err){
		exit(0);
